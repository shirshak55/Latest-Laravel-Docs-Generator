...
- [Registering Events & Listeners](#events-registering-events-and-listeners)
    - [Generating Events & Listeners](#events-generating-events-and-listeners)
    - [Manually Registering Events](#events-manually-registering-events)
- [Defining Events](#events-defining-events)
- [Defining Listeners](#events-defining-listeners)
- [Queued Event Listeners](#events-queued-event-listeners)
    - [Manually Accessing The Queue](#events-manually-accessing-the-queue)
    - [Handling Failed Jobs](#events-handling-failed-jobs)
- [Dispatching Events](#events-dispatching-events)
- [Event Subscribers](#events-event-subscribers)
    - [Writing Event Subscribers](#events-writing-event-subscribers)
    - [Registering Event Subscribers](#events-registering-event-subscribers)
...
An event class is a data container which holds the information related to the event. For example, let's assume our generated `OrderShipped` event receives an [Eloquent ORM](#{{version}}/eloquent) object:
...
> {tip} Your event listeners may also type-hint any dependencies they need on their constructors. All event listeners are resolved via the Laravel [service container](#{{version}}/container), so dependencies will be injected automatically.
...
Queueing listeners can be beneficial if your listener is going to perform a slow task such as sending an e-mail or making an HTTP request. Before getting started with queued listeners, make sure to [configure your queue](#{{version}}/queues) and start a queue listener on your server or local development environment.
...
That's it! Now, when this listener is called for an event, it will be automatically queued by the event dispatcher using Laravel's [queue system](#{{version}}/queues). If no exceptions are thrown when the listener is executed by the queue, the queued job will automatically be deleted after it has finished processing.
...
> {tip} When testing, it can be helpful to assert that certain events were dispatched without actually triggering their listeners. Laravel's [built-in testing helpers](#{{version}}/mocking-event-fake) makes it a cinch.
