...
- [Introduction](#notifications-introduction)
- [Creating Notifications](#notifications-creating-notifications)
- [Sending Notifications](#notifications-sending-notifications)
    - [Using The Notifiable Trait](#notifications-using-the-notifiable-trait)
    - [Using The Notification Facade](#notifications-using-the-notification-facade)
    - [Specifying Delivery Channels](#notifications-specifying-delivery-channels)
    - [Queueing Notifications](#notifications-queueing-notifications)
    - [On-Demand Notifications](#notifications-on-demand-notifications)
- [Mail Notifications](#notifications-mail-notifications)
    - [Formatting Mail Messages](#notifications-formatting-mail-messages)
    - [Customizing The Recipient](#notifications-customizing-the-recipient)
    - [Customizing The Subject](#notifications-customizing-the-subject)
    - [Customizing The Templates](#notifications-customizing-the-templates)
- [Markdown Mail Notifications](#notifications-markdown-mail-notifications)
    - [Generating The Message](#notifications-generating-the-message)
    - [Writing The Message](#notifications-writing-the-message)
    - [Customizing The Components](#notifications-customizing-the-components)
- [Database Notifications](#notifications-database-notifications)
    - [Prerequisites](#notifications-database-prerequisites)
    - [Formatting Database Notifications](#notifications-formatting-database-notifications)
    - [Accessing The Notifications](#notifications-accessing-the-notifications)
    - [Marking Notifications As Read](#notifications-marking-notifications-as-read)
- [Broadcast Notifications](#notifications-broadcast-notifications)
    - [Prerequisites](#notifications-broadcast-prerequisites)
    - [Formatting Broadcast Notifications](#notifications-formatting-broadcast-notifications)
    - [Listening For Notifications](#notifications-listening-for-notifications)
- [SMS Notifications](#notifications-sms-notifications)
    - [Prerequisites](#notifications-sms-prerequisites)
    - [Formatting SMS Notifications](#notifications-formatting-sms-notifications)
    - [Customizing The "From" Number](#notifications-customizing-the-from-number)
    - [Routing SMS Notifications](#notifications-routing-sms-notifications)
- [Slack Notifications](#notifications-slack-notifications)
    - [Prerequisites](#notifications-slack-prerequisites)
    - [Formatting Slack Notifications](#notifications-formatting-slack-notifications)
    - [Slack Attachments](#notifications-slack-attachments)
    - [Routing Slack Notifications](#notifications-routing-slack-notifications)
- [Notification Events](#notifications-notification-events)
- [Custom Channels](#notifications-custom-channels)

## Introduction {#notifications-introduction}

In addition to support for [sending email](#{{version}}/mail), Laravel provides support for sending notifications across a variety of delivery channels, including mail, SMS (via [Nexmo](https://www.nexmo.com/)), and [Slack](https://slack.com). Notifications may also be stored in a database so they may be displayed in your web interface.
...
Notifications may be sent in two ways: using the `notify` method of the `Notifiable` trait or using the `Notification` [facade](#{{version}}/facades). First, let's explore using the trait:
...
Alternatively, you may send notifications via the `Notification` [facade](#{{version}}/facades). This is useful primarily when you need to send a notification to multiple notifiable entities such as a collection of users. To send notifications using the facade, pass all of the notifiable entities and the notification instance to the `send` method:
...
> {note} Before queueing notifications you should configure your queue and [start a worker](#{{version}}/queues).
...
In addition, you may return a [mailable object](#{{version}}/mail) from the `toMail` method:
...
#### Error Messages {#notifications-error-messages}
...
Before broadcasting notifications, you should configure and be familiar with Laravel's [event broadcasting](#{{version}}/broadcasting) services. Event broadcasting provides a way to react to server-side fired Laravel events from your JavaScript client.

### Formatting Broadcast Notifications {#notifications-formatting-broadcast-notifications}

The `broadcast` channel broadcasts notifications using Laravel's [event broadcasting](#{{version}}/broadcasting) services, allowing your JavaScript client to catch notifications in realtime. If a notification supports broadcasting, you should define a `toBroadcast` method on the notification class. This method will receive a `$notifiable` entity and should return a `BroadcastMessage` instance. The returned data will be encoded as JSON and broadcast to your JavaScript client. Let's take a look at an example `toBroadcast` method:
...
You will also need to configure an ["Incoming Webhook"](https://api.slack.com/incoming-webhooks) integration for your Slack team. This integration will provide you with a URL you may use when [routing Slack notifications](#notifications-routing-slack-notifications).
