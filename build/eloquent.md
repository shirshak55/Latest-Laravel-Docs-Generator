...
- [Introduction](#eloquent-introduction)
- [Defining Models](#eloquent-defining-models)
    - [Eloquent Model Conventions](#eloquent-eloquent-model-conventions)
- [Retrieving Models](#eloquent-retrieving-models)
    - [Collections](#eloquent-collections)
    - [Chunking Results](#eloquent-chunking-results)
- [Retrieving Single Models / Aggregates](#eloquent-retrieving-single-models)
    - [Retrieving Aggregates](#eloquent-retrieving-aggregates)
- [Inserting & Updating Models](#eloquent-inserting-and-updating-models)
    - [Inserts](#eloquent-inserts)
    - [Updates](#eloquent-updates)
    - [Mass Assignment](#eloquent-mass-assignment)
    - [Other Creation Methods](#eloquent-other-creation-methods)
- [Deleting Models](#eloquent-deleting-models)
    - [Soft Deleting](#eloquent-soft-deleting)
    - [Querying Soft Deleted Models](#eloquent-querying-soft-deleted-models)
- [Query Scopes](#eloquent-query-scopes)
    - [Global Scopes](#eloquent-global-scopes)
    - [Local Scopes](#eloquent-local-scopes)
- [Comparing Models](#eloquent-comparing-models)
- [Events](#eloquent-events)
...
Before getting started, be sure to configure a database connection in `config/database.php`. For more information on configuring your database, check out [the documentation](#{{version}}/database-configuration).
...
The easiest way to create a model instance is using the `make:model` [Artisan command](#{{version}}/artisan):

    php artisan make:model Flight

If you would like to generate a [database migration](#{{version}}/migrations) when you generate the model, you may use the `--migration` or `-m` option:
...
Once you have created a model and [its associated database table](#{{version}}/migrations-writing-migrations), you are ready to start retrieving data from your database. Think of each Eloquent model as a powerful [query builder](#{{version}}/queries) allowing you to fluently query the database table associated with the model. For example:
...
The Eloquent `all` method will return all of the results in the model's table. Since each Eloquent model serves as a [query builder](#{{version}}/queries), you may also add constraints to queries, and then use the `get` method to retrieve the results:
...
> {tip} Since Eloquent models are query builders, you should review all of the methods available on the [query builder](#{{version}}/queries). You may use any of these methods in your Eloquent queries.

### Collections {#eloquent-collections}

For Eloquent methods like `all` and `get` which retrieve multiple results, an instance of `Illuminate\Database\Eloquent\Collection` will be returned. The `Collection` class provides [a variety of helpful methods](#{{version}}/eloquent-collections-available-methods) for working with your Eloquent results:
...
You may also use the `count`, `sum`, `max`, and other [aggregate methods](#{{version}}/queries-aggregates) provided by the [query builder](#{{version}}/queries). These methods return the appropriate scalar value instead of a full model instance:
...
Of course, you should add the `deleted_at` column to your database table. The Laravel [schema builder](#{{version}}/migrations) contains a helper method to create this column:
...
The `withTrashed` method may also be used on a [relationship](#{{version}}/eloquent-relationships) query:
...
Like the `withTrashed` method, the `restore` method may also be used on [relationships](#{{version}}/eloquent-relationships):
...
To get started, define a `$dispatchesEvents` property on your Eloquent model that maps various points of the Eloquent model's lifecycle to your own [event classes](#{{version}}/events):
