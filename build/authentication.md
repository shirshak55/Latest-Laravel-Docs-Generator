...
- [Introduction](#authentication-introduction)
    - [Database Considerations](#authentication-introduction-database-considerations)
- [Authentication Quickstart](#authentication-authentication-quickstart)
    - [Routing](#authentication-included-routing)
    - [Views](#authentication-included-views)
    - [Authenticating](#authentication-included-authenticating)
    - [Retrieving The Authenticated User](#authentication-retrieving-the-authenticated-user)
    - [Protecting Routes](#authentication-protecting-routes)
    - [Login Throttling](#authentication-login-throttling)
- [Manually Authenticating Users](#authentication-authenticating-users)
    - [Remembering Users](#authentication-remembering-users)
    - [Other Authentication Methods](#authentication-other-authentication-methods)
- [HTTP Basic Authentication](#authentication-http-basic-authentication)
    - [Stateless HTTP Basic Authentication](#authentication-stateless-http-basic-authentication)
- [Logging Out](#authentication-logging-out)
    - [Invalidating Sessions On Other Devices](#authentication-invalidating-sessions-on-other-devices)
- [Social Authentication](https://github.com/laravel/socialite)
- [Adding Custom Guards](#authentication-adding-custom-guards)
    - [Closure Request Guards](#authentication-closure-request-guards)
- [Adding Custom User Providers](#authentication-adding-custom-user-providers)
    - [The User Provider Contract](#authentication-the-user-provider-contract)
    - [The Authenticatable Contract](#authentication-the-authenticatable-contract)
...
The `create` method of the `RegisterController` is responsible for creating new `App\User` records in your database using the [Eloquent ORM](#{{version}}/eloquent). You are free to modify this method according to the needs of your database.
...
Of course, if you are using [controllers](#{{version}}/controllers), you may call the `middleware` method from the controller's constructor instead of attaching it in the route definition directly:
...
When the `auth` middleware detects an unauthorized user, it will either return a JSON `401` response, or, if the request was not an AJAX request, redirect the user to the `login` [named route](#{{version}}/routing-named-routes).
...
You may define your own authentication guards using the `extend` method on the `Auth` facade. You should place this call to `extend` within a [service provider](#{{version}}/providers). Since Laravel already ships with an `AuthServiceProvider`, we can place the code in that provider:
