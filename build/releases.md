...
- [Versioning Scheme](#releases-versioning-scheme)
- [Support Policy](#releases-support-policy)
- [Laravel 5.6](#releases-laravel-5.6)
...
In addition, it is now easier to customize existing log channels using the logging system's new "tap" functionality. For more information, check out the [full documentation on logging](#{{version}}/logging).
...
When specifying a [rate limit](#{{version}}/routing-rate-limiting) on a group of routes in previous releases of Laravel, you were forced to provide a hard-coded number of maximum requests:
