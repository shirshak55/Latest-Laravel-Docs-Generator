...
- [Introduction](#database-introduction)
    ...
    - [Read & Write Connections](#database-read-and-write-connections)
    - [Using Multiple Database Connections](#database-using-multiple-database-connections)
- [Running Raw SQL Queries](#database-running-queries)
    - [Listening For Query Events](#database-listening-for-query-events)
- [Database Transactions](#database-database-transactions)

## Introduction {#database-introduction}

Laravel makes interacting with databases extremely simple across a variety of database backends using either raw SQL, the [fluent query builder](#{{version}}/queries), and the [Eloquent ORM](#{{version}}/eloquent). Currently, Laravel supports four databases:
...
By default, Laravel's sample [environment configuration](#{{version}}/configuration-environment-configuration) is ready to use with [Laravel Homestead](#{{version}}/homestead), which is a convenient virtual machine for doing Laravel development on your local machine. Of course, you are free to modify this configuration as needed for your local database.
...
> {tip} The `DB` facade's transaction methods control the transactions for both the [query builder](#{{version}}/queries) and [Eloquent ORM](#{{version}}/eloquent).
