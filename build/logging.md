...
- [Introduction](#logging-introduction)
- [Configuration](#logging-configuration)
    - [Building Log Stacks](#logging-building-log-stacks)
- [Writing Log Messages](#logging-writing-log-messages)
    - [Writing To Specific Channels](#logging-writing-to-specific-channels)
- [Advanced Monolog Channel Customization](#logging-advanced-monolog-channel-customization)
    - [Customizing Monolog For Channels](#logging-customizing-monolog-for-channels)
    - [Creating Monolog Handler Channels](#logging-creating-monolog-handler-channels)
    - [Creating Channels Via Factories](#logging-creating-channels-via-factories)
...
By default, Laravel will use the `stack` channel when logging messages. The `stack` channel is used to aggregate multiple log channels into a single channel. For more information on building stacks, check out the [documentation below](#logging-building-log-stacks).
...
> {tip} Check out the documentation on [advanced channel customization](#logging-advanced-monolog-channel-customization) to learn more about the `monolog` and `custom` drivers.
...
You may write information to the logs using the `Log` [facade](#{{version}}/facades). As previously mentioned, the logger provides the eight logging levels defined in the [RFC 5424 specification](https://tools.ietf.org/html/rfc5424): **emergency**, **alert**, **critical**, **error**, **warning**, **notice**, **info** and **debug**:
...
> {tip} All of your "tap" classes are resolved by the [service container](#{{version}}/container), so any constructor dependencies they require will automatically be injected.
