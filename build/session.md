...
- [Introduction](#session-introduction)
    ...
    - [Driver Prerequisites](#session-driver-prerequisites)
- [Using The Session](#session-using-the-session)
    - [Retrieving Data](#session-retrieving-data)
    - [Storing Data](#session-storing-data)
    - [Flash Data](#session-flash-data)
    - [Deleting Data](#session-deleting-data)
    - [Regenerating The Session ID](#session-regenerating-the-session-id)
- [Adding Custom Session Drivers](#session-adding-custom-session-drivers)
    - [Implementing The Driver](#session-implementing-the-driver)
    - [Registering The Driver](#session-registering-the-driver)
...
> {tip} The array driver is used during [testing](#{{version}}/testing) and prevents the data stored in the session from being persisted.
...
> {tip} There is little practical difference between using the session via an HTTP request instance versus using the global `session` helper. Both methods are [testable](#{{version}}/testing) via the `assertSessionHas` method which is available in all of your test cases.
...
Once your driver has been implemented, you are ready to register it with the framework. To add additional drivers to Laravel's session backend, you may use the `extend` method on the `Session` [facade](#{{version}}/facades). You should call the `extend` method from the `boot` method of a [service provider](#{{version}}/providers). You may do this from the existing `AppServiceProvider` or create an entirely new provider:
