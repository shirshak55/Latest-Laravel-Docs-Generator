...
- [Binding](#container-binding)
    - [Binding Basics](#container-binding-basics)
    - [Binding Interfaces To Implementations](#container-binding-interfaces-to-implementations)
    - [Contextual Binding](#container-contextual-binding)
    - [Tagging](#container-tagging)
    - [Extending Bindings](#container-extending-bindings)
- [Resolving](#container-resolving)
    - [The Make Method](#container-the-make-method)
    - [Automatic Injection](#container-automatic-injection)
- [Container Events](#container-container-events)
- [PSR-11](#container-psr-11)
...
In this example, the `UserController` needs to retrieve users from a data source. So, we will **inject** a service that is able to retrieve users. In this context, our `UserRepository` most likely uses [Eloquent](#{{version}}/eloquent) to retrieve user information from the database. However, since the repository is injected, we are able to easily swap it out with another implementation. We are also able to easily "mock", or create a dummy implementation of the `UserRepository` when testing our application.
...
Almost all of your service container bindings will be registered within [service providers](#{{version}}/providers), so most of these examples will demonstrate using the container in that context.
...
Sometimes you may have two classes that utilize the same interface, but you wish to inject different implementations into each class. For example, two controllers may depend on different implementations of the `Illuminate\Contracts\Filesystem\Filesystem` [contract](#{{version}}/contracts). Laravel provides a simple, fluent interface for defining this behavior:
