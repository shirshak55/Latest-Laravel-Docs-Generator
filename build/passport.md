# API Authentication (Passport) {#passport}

- [Introduction](#passport-introduction)
- [Installation](#passport-installation)
    - [Frontend Quickstart](#passport-frontend-quickstart)
    - [Deploying Passport](#passport-deploying-passport)
- [Configuration](#passport-configuration)
    - [Token Lifetimes](#passport-token-lifetimes)
- [Issuing Access Tokens](#passport-issuing-access-tokens)
    - [Managing Clients](#passport-managing-clients)
    - [Requesting Tokens](#passport-requesting-tokens)
    - [Refreshing Tokens](#passport-refreshing-tokens)
- [Password Grant Tokens](#passport-password-grant-tokens)
    - [Creating A Password Grant Client](#passport-creating-a-password-grant-client)
    - [Requesting Tokens](#passport-requesting-password-grant-tokens)
    - [Requesting All Scopes](#passport-requesting-all-scopes)
- [Implicit Grant Tokens](#passport-implicit-grant-tokens)
- [Client Credentials Grant Tokens](#passport-client-credentials-grant-tokens)
- [Personal Access Tokens](#passport-personal-access-tokens)
    - [Creating A Personal Access Client](#passport-creating-a-personal-access-client)
    - [Managing Personal Access Tokens](#passport-managing-personal-access-tokens)
- [Protecting Routes](#passport-protecting-routes)
    - [Via Middleware](#passport-via-middleware)
    - [Passing The Access Token](#passport-passing-the-access-token)
- [Token Scopes](#passport-token-scopes)
    - [Defining Scopes](#passport-defining-scopes)
    - [Assigning Scopes To Tokens](#passport-assigning-scopes-to-tokens)
    - [Checking Scopes](#passport-checking-scopes)
- [Consuming Your API With JavaScript](#passport-consuming-your-api-with-javascript)
- [Events](#passport-events)
- [Testing](#passport-testing)
...
> {tip} If you don't want to implement the entire client management frontend yourself, you can use the [frontend quickstart](#passport-frontend-quickstart) to have a fully functional frontend in a matter of minutes.
...
> {tip} Remember, access tokens are long-lived by default. However, you are free to [configure your maximum access token lifetime](#passport-configuration) if needed.
...
Once you have created a personal access client, you may issue tokens for a given user using the `createToken` method on the `User` model instance. The `createToken` method accepts the name of the token as its first argument and an optional array of [scopes](#passport-token-scopes) as its second argument:
...
> {tip} If you don't want to implement the personal access token frontend yourself, you can use the [frontend quickstart](#passport-frontend-quickstart) to have a fully functional frontend in a matter of minutes.

#### `GET /oauth/scopes`

This route returns all of the [scopes](#passport-token-scopes) defined for your application. You may use this route to list the scopes a user may assign to a personal access token:
...
When using this method of authentication, the default Laravel JavaScript scaffolding instructs Axios to always send the `X-CSRF-TOKEN` and `X-Requested-With` headers. However, you should be sure to include your CSRF token in a [HTML meta tag](#{{version}}/csrf-csrf-x-csrf-token):
