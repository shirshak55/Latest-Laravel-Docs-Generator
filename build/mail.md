...
- [Introduction](#mail-introduction)
    - [Driver Prerequisites](#mail-driver-prerequisites)
- [Generating Mailables](#mail-generating-mailables)
- [Writing Mailables](#mail-writing-mailables)
    - [Configuring The Sender](#mail-configuring-the-sender)
    - [Configuring The View](#mail-configuring-the-view)
    - [View Data](#mail-view-data)
    - [Attachments](#mail-attachments)
    - [Inline Attachments](#mail-inline-attachments)
    - [Customizing The SwiftMailer Message](#mail-customizing-the-swiftmailer-message)
- [Markdown Mailables](#mail-markdown-mailables)
    - [Generating Markdown Mailables](#mail-generating-markdown-mailables)
    - [Writing Markdown Messages](#mail-writing-markdown-messages)
    - [Customizing The Components](#mail-customizing-the-components)
- [Sending Mail](#mail-sending-mail)
    - [Queueing Mail](#mail-queueing-mail)
- [Rendering Mailables](#mail-rendering-mailables)
    - [Previewing Mailables In The Browser](#mail-previewing-mailables-in-the-browser)
- [Mail & Local Development](#mail-mail-and-local-development)
- [Events](#mail-events)
...
Within a mailable class' `build` method, you may use the `view` method to specify which template should be used when rendering the email's contents. Since each email typically uses a [Blade template](#{{version}}/blade) to render its contents, you have the full power and convenience of the Blade templating engine when building your email's HTML:
...
To send a message, use the `to` method on the `Mail` [facade](#{{version}}/facades). The `to` method accepts an email address, a user instance, or a collection of users. If you pass an object or collection of objects, the mailer will automatically use their `email` and `name` properties when setting the email recipients, so make sure these attributes are available on your objects. Once you have specified your recipients, you may pass an instance of your mailable class to the `send` method:
...
This method will automatically take care of pushing a job onto the queue so the message is sent in the background. Of course, you will need to [configure your queues](#{{version}}/queues) before using this feature.
...
Instead of sending your emails, the `log` mail driver will write all email messages to your log files for inspection. For more information on configuring your application per environment, check out the [configuration documentation](#{{version}}/configuration-environment-configuration).
