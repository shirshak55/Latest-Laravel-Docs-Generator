...
- [Introduction](#billing-introduction)
- [Configuration](#billing-configuration)
    - [Stripe](#billing-stripe-configuration)
    - [Braintree](#billing-braintree-configuration)
    - [Currency Configuration](#billing-currency-configuration)
- [Subscriptions](#billing-subscriptions)
    - [Creating Subscriptions](#billing-creating-subscriptions)
    - [Checking Subscription Status](#billing-checking-subscription-status)
    - [Changing Plans](#billing-changing-plans)
    - [Subscription Quantity](#billing-subscription-quantity)
    - [Subscription Taxes](#billing-subscription-taxes)
    - [Cancelling Subscriptions](#billing-cancelling-subscriptions)
    - [Resuming Subscriptions](#billing-resuming-subscriptions)
    - [Updating Credit Cards](#billing-updating-credit-cards)
- [Subscription Trials](#billing-subscription-trials)
    - [With Credit Card Up Front](#billing-with-credit-card-up-front)
    - [Without Credit Card Up Front](#billing-without-credit-card-up-front)
- [Handling Stripe Webhooks](#billing-handling-stripe-webhooks)
    - [Defining Webhook Event Handlers](#billing-defining-webhook-event-handlers)
    - [Failed Subscriptions](#billing-handling-failed-subscriptions)
- [Handling Braintree Webhooks](#billing-handling-braintree-webhooks)
    - [Defining Webhook Event Handlers](#billing-defining-braintree-webhook-event-handlers)
    - [Failed Subscriptions](#billing-handling-braintree-failed-subscriptions)
- [Single Charges](#billing-single-charges)
- [Invoices](#billing-invoices)
    - [Generating Invoice PDFs](#billing-generating-invoice-pdfs)
...
Before using Cashier, we'll also need to [prepare the database](#{{version}}/migrations). We need to add several columns to your `users` table and create a new `subscriptions` table to hold all of our customer's subscriptions:
...
Next, register the `Laravel\Cashier\CashierServiceProvider` [service provider](#{{version}}/providers) in your `config/app.php` configuration file:
...
Before using Cashier, we'll need to [prepare the database](#{{version}}/migrations). We need to add several columns to your `users` table and create a new `subscriptions` table to hold all of our customer's subscriptions:
...
> {note}  Be sure to add a [date mutator](#{{version}}/eloquent-mutators-date-mutators) for `trial_ends_at` to your model definition.
...
Since Stripe webhooks need to bypass Laravel's [CSRF protection](#{{version}}/csrf), be sure to list the URI as an exception in your `VerifyCsrfToken` middleware or list the route outside of the `web` middleware group:
...
Since Braintree webhooks need to bypass Laravel's [CSRF protection](#{{version}}/csrf), be sure to list the URI as an exception in your `VerifyCsrfToken` middleware or list the route outside of the `web` middleware group:
