...
- [Configuration](#cache-configuration)
    - [Driver Prerequisites](#cache-driver-prerequisites)
- [Cache Usage](#cache-cache-usage)
    - [Obtaining A Cache Instance](#cache-obtaining-a-cache-instance)
    - [Retrieving Items From The Cache](#cache-retrieving-items-from-the-cache)
    - [Storing Items In The Cache](#cache-storing-items-in-the-cache)
    - [Removing Items From The Cache](#cache-removing-items-from-the-cache)
    - [Atomic Locks](#cache-atomic-locks)
    - [The Cache Helper](#cache-the-cache-helper)
- [Cache Tags](#cache-cache-tags)
    - [Storing Tagged Cache Items](#cache-storing-tagged-cache-items)
    - [Accessing Tagged Cache Items](#cache-accessing-tagged-cache-items)
    - [Removing Tagged Cache Items](#cache-removing-tagged-cache-items)
- [Adding Custom Cache Drivers](#cache-adding-custom-cache-drivers)
    - [Writing The Driver](#cache-writing-the-driver)
    - [Registering The Driver](#cache-registering-the-driver)
- [Events](#cache-events)
...
For more information on configuring Redis, consult its [Laravel documentation page](#{{version}}/redis-configuration).

## Cache Usage {#cache-cache-usage}

### Obtaining A Cache Instance {#cache-obtaining-a-cache-instance}

The `Illuminate\Contracts\Cache\Factory` and `Illuminate\Contracts\Cache\Repository` [contracts](#{{version}}/contracts) provide access to Laravel's cache services. The `Factory` contract provides access to all cache drivers defined for your application. The `Repository` contract is typically an implementation of the default cache driver for your application as specified by your `cache` configuration file.
...
In addition to using the `Cache` facade or [cache contract](#{{version}}/contracts), you may also use the global `cache` function to retrieve and store data via the cache. When the `cache` function is called with a single, string argument, it will return the value of the given key:
...
> {tip} When testing call to the global `cache` function, you may use the `Cache::shouldReceive` method just as if you were [testing a facade](#{{version}}/mocking-mocking-facades).
...
To create our custom cache driver, we first need to implement the `Illuminate\Contracts\Cache\Store` [contract](#{{version}}/contracts). So, a MongoDB cache implementation would look something like this:
...
The first argument passed to the `extend` method is the name of the driver. This will correspond to your `driver` option in the `config/cache.php` configuration file. The second argument is a Closure that should return an `Illuminate\Cache\Repository` instance. The Closure will be passed an `$app` instance, which is an instance of the [service container](#{{version}}/container).
...
To execute code on every cache operation, you may listen for the [events](#{{version}}/events) fired by the cache. Typically, you should place these event listeners within your `EventServiceProvider`:
