...
- [Introduction](#queues-introduction)
    - [Connections Vs. Queues](#queues-connections-vs-queues)
    - [Driver Notes & Prerequisites](#queues-driver-prerequisites)
- [Creating Jobs](#queues-creating-jobs)
    - [Generating Job Classes](#queues-generating-job-classes)
    - [Class Structure](#queues-class-structure)
- [Dispatching Jobs](#queues-dispatching-jobs)
    - [Delayed Dispatching](#queues-delayed-dispatching)
    - [Job Chaining](#queues-job-chaining)
    - [Customizing The Queue & Connection](#queues-customizing-the-queue-and-connection)
    - [Specifying Max Job Attempts / Timeout Values](#queues-max-job-attempts-and-timeout)
    - [Rate Limiting](#queues-rate-limiting)
    - [Error Handling](#queues-error-handling)
- [Running The Queue Worker](#queues-running-the-queue-worker)
    - [Queue Priorities](#queues-queue-priorities)
    - [Queue Workers & Deployment](#queues-queue-workers-and-deployment)
    - [Job Expirations & Timeouts](#queues-job-expirations-and-timeouts)
- [Supervisor Configuration](#queues-supervisor-configuration)
- [Dealing With Failed Jobs](#queues-dealing-with-failed-jobs)
    - [Cleaning Up After Failed Jobs](#queues-cleaning-up-after-failed-jobs)
    - [Failed Job Events](#queues-failed-job-events)
    - [Retrying Failed Jobs](#queues-retrying-failed-jobs)
- [Job Events](#queues-job-events)

## Introduction {#queues-introduction}

> {tip} Laravel now offers Horizon, a beautiful dashboard and configuration system for your Redis powered queues. Check out the full [Horizon documentation](#{{version}}/horizon) for more information.
...
In this example, note that we were able to pass an [Eloquent model](#{{version}}/eloquent) directly into the queued job's constructor. Because of the `SerializesModels` trait that the job is using, Eloquent models will be gracefully serialized and unserialized when the job is processing. If your queued job accepts an Eloquent model in its constructor, only the identifier for the model will be serialized onto the queue. When the job is actually handled, the queue system will automatically re-retrieve the full model instance from the database. It's all totally transparent to your application and prevents issues that can arise from serializing full Eloquent model instances.

The `handle` method is called when the job is processed by the queue. Note that we are able to type-hint dependencies on the `handle` method of the job. The Laravel [service container](#{{version}}/container) automatically injects these dependencies.
...
#### Time Based Attempts {#queues-time-based-attempts}
...
> {note} This feature requires that your application can interact with a [Redis server](#{{version}}/redis).
...
> {tip} When using rate limiting, the number of attempts your job will need to run successfully can be hard to determine. Therefore, it is useful to combine rate limiting with [time based attempts](#queues-time-based-attempts).

### Error Handling {#queues-error-handling}

If an exception is thrown while the job is being processed, the job will automatically be released back onto the queue so it may be attempted again. The job will continue to be released until it has been attempted the maximum number of times allowed by your application. The maximum number of attempts is defined by the `--tries` switch used on the `queue:work` Artisan command. Alternatively, the maximum number of attempts may be defined on the job class itself. More information on running the queue worker [can be found below](#queues-running-the-queue-worker).
...
> {tip} To keep the `queue:work` process running permanently in the background, you should use a process monitor such as [Supervisor](#queues-supervisor-configuration) to ensure that the queue worker does not stop running.

Remember, queue workers are long-lived processes and store the booted application state in memory. As a result, they will not notice changes in your code base after they have been started. So, during your deployment process, be sure to [restart your queue workers](#queues-queue-workers-and-deployment).
...
This command will instruct all queue workers to gracefully "die" after they finish processing their current job so that no existing jobs are lost. Since the queue workers will die when the `queue:restart` command is executed, you should be running a process manager such as [Supervisor](#queues-supervisor-configuration) to automatically restart the queue workers.

> {tip} The queue uses the [cache](#{{version}}/cache) to store restart signals, so you should verify a cache driver is properly configured for your application before using this feature.
...
Then, when running your [queue worker](#queues-running-the-queue-worker), you should specify the maximum number of times a job should be attempted using the `--tries` switch on the `queue:work` command. If you do not specify a value for the `--tries` option, jobs will be attempted indefinitely:
...
Using the `before` and `after` methods on the `Queue` [facade](#{{version}}/facades), you may specify callbacks to be executed before or after a queued job is processed. These callbacks are a great opportunity to perform additional logging or increment statistics for a dashboard. Typically, you should call these methods from a [service provider](#{{version}}/providers). For example, we may use the `AppServiceProvider` that is included with Laravel:
...
Using the `looping` method on the `Queue` [facade](#{{version}}/facades), you may specify callbacks that execute before the worker attempts to fetch a job from a queue. For example, you might register a Closure to rollback any transactions that were left open by a previously failed job:
